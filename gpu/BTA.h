...
//	}
	...
	this->t.start();
	local_sort2<T,Z><<<_grid,_block>>>(this->gdata,this->n,qq,k,this->gscores);
...
//	std::cout << std::endl;
	for(uint32_t i = 0; i < 32; i++){ std::cout << this->cscores[i] << std::endl; if((i+1)%k ==0 ){ std::cout << "-----" <<std::endl;}}
	//this->cpu_threshold = find_threshold<T,Z>(this->cscores,this->n,k);
	this->cpu_threshold = find_partial_threshold<T,Z>(this->cscores,this->n,k);
}
