...
struct bta_block{
	...
	T tuples[BLOCK_SIZE * NUM_DIMS] __attribute__((aligned(32)));
};
...
class BTA : public AA<T,Z>{
	...
	private:
		bta_partition<T,Z> parts[PARTITIONS];
};
...
void BTA<T,Z>::init(){
	...
	for(uint64_t i = 0; i < PARTITIONS; i++){
		uint64_t first = ((i*this->n)/PARTITIONS);
		uint64_t last = (((i+1)*this->n)/PARTITIONS) - 1;
		parts[i].offset = part_offset;
		parts[i].size = last - first + 1;
		parts[i].block_num = ((parts[i].size - 1) / BLOCK_SIZE) + 1;
	...
	uint64_t max_part_size = (((this->n - 1)/PARTITIONS) + 1);
...
//		}
		...
		for(uint64_t j = 0; j < parts[i].size; ){
			...
			for(jj = 0; jj < parts[i].blocks[bnum].tuple_num; jj++){//For each block//
				...
				for(uint8_t m = 0; m < this->d; m++){
					parts[i].blocks[bnum].tuples[m*BLOCK_SIZE + jj] = this->cdata[m*this->n + (poffset + id)];
//					if(jj >= BLOCK_SIZE){
		...
		for(uint64_t b = 0; b < parts[i].block_num; b++){
			...
			for(uint64_t t = 0; t < tuple_num; t+=8){
				...
				for(uint8_t m = 0; m < qq; m++){
					uint32_t offset = m*BLOCK_SIZE + t;
	...
	for(uint64_t i = 0; i < PARTITIONS; i++){
		for(uint64_t b = 0; b < parts[i].block_num; b++){
			...
			for(uint64_t t = 0; t < tuple_num; t+=16){
				...
				for(uint8_t m = 0; m < qq; m++){
					uint64_t offset = m*BLOCK_SIZE + t;
