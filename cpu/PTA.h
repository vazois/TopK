...
void PTA<T,Z>::findTopKscalar(uint64_t k, uint8_t qq, T *weights, uint8_t *attr){
	...
	for(uint64_t p = 0; p < PPARTITIONS; p++){
		...
		for(uint64_t b = 0; b < this->parts[p].block_num; b++){
			...
			for(uint64_t t = 0; t < this->parts[p].blocks[b].tuple_num; t+=1){
				id+=t;
				T score00 = 0;
//				T score01 = 0;
//				T score02 = 0;
...
//					if(q.top().score < score07){ q.pop(); q.push(tuple_<T,Z>(id+7,score07)); }
					if(STATS_EFF) this->accesses+=1*2;
				}
...
void PTA<T,Z>::findTopKsimd(uint64_t k, uint8_t qq, T *weights, uint8_t *attr){
	...
	for(uint64_t p = 0; p < PPARTITIONS; p++){
		...
		for(uint64_t b = 0; b < this->parts[p].block_num; b++){
			...
			T *tuples = this->parts[p].blocks[b].tuples;
			if(STATS_EFF) this->accesses+=1;
			for(uint64_t t = 0; t < this->parts[p].blocks[b].tuple_num; t+=32){
				id+=t;
...
//					score07 = _mm256_add_ps(score07,_mm256_mul_ps(_mm256_load_ps(&tuples[offset+56]),_weight));
				...
				if(STATS_EFF) this->accesses+=qq*35;//3 accesses plus 32
