...
inline uint32_t DT_bitmap_dvc( const TUPLE &cur, const TUPLE &sky ) {
  ...
  for (; dim+8 <= NUM_DIMS; dim+=8) {
    ...
    __m256 comp_ge = _mm256_cmp_ps(sky_ymm, p_ymm, 13);
    uint32_t ge_mask = _mm256_movemask_ps(comp_ge);
    lattice = lattice | (ge_mask << dim);
  }
  ...
  for (; dim+4 <= NUM_DIMS; dim+=4) {
    ...
    __m128 ge128 = _mm_cmp_ps(sky_xmm, p_xmm, 13);
    uint32_t ge_mask = _mm_movemask_ps(ge128);
    lattice = lattice | (ge_mask << dim);
  }
...
//  if ( sky.elems[dim] <= cur.elems[dim] )
  ...
  for (; dim + 4 <= NUM_DIMS; dim += 4) {
    ...
    __m128 comp_ge = _mm_cmp_ps( sky_xmm, p_xmm, 13 );
    uint32_t ge_mask = _mm_movemask_ps( comp_ge );
    lattice = lattice | (ge_mask << dim);
  }
...
inline uint32_t DT_bitmap( const TUPLE cur, const TUPLE sky ) {
  ...
  for (; dim+8 <= NUM_DIMS; dim+=8) {
    ...
    __m256 comp_gt = _mm256_cmp_ps(sky_ymm, p_ymm, 14);
    uint32_t gt_mask = _mm256_movemask_ps(comp_gt);
    lattice = lattice | (gt_mask << dim);
  }
...
//    __m128 comp_lt = _mm_cmp_ps(sky_xmm, p_xmm, 1);
    __m128 comp_gt = _mm_cmp_ps(sky_xmm, p_xmm, 14);
    uint32_t gt_mask = _mm_movemask_ps(comp_gt);
    lattice = lattice | (gt_mask << dim);
  }
...
inline int DominanceTest( const TUPLE &left, const TUPLE &right ) {
  ...
  for (; dim+8 <= NUM_DIMS; dim+=8) {
    ...
    if ( !left_better ) {
      ...
      __m256 comp_gt = _mm256_cmp_ps(left_ymm, right_ymm, 14);
      uint32_t gt_mask = _mm256_movemask_ps(comp_gt);
      left_better = gt_mask & (SHIFTS[8] - 1);
    }
    if ( !right_better ) {
      //__m256 comp_lt = _mm256_cmp_ps(right_ymm, left_ymm, 1);
      __m256 comp_gt = _mm256_cmp_ps(right_ymm, left_ymm, 14);
      uint32_t gt_mask = _mm256_movemask_ps(comp_gt);
      right_better = gt_mask & (SHIFTS[8] - 1);
    }
  ...
  for (; dim+4 <= NUM_DIMS; dim+=4) {
    ...
    if ( !left_better ) {
      ...
      __m128 comp_gt = _mm_cmp_ps(left_ymm, right_ymm, 14);
      uint32_t gt_mask = _mm_movemask_ps(comp_gt);
      left_better = gt_mask & (SHIFTS[4] - 1);
    }
    if ( !right_better ) {
      //__m128 comp_lt = _mm_cmp_ps(right_ymm, left_ymm, 1);
      __m128 comp_gt = _mm_cmp_ps(right_ymm, left_ymm, 14);
      uint32_t gt_mask = _mm_movemask_ps(comp_gt);
      right_better = gt_mask & (SHIFTS[4] - 1);
    }
    if( left_better && right_better ) return DOM_INCOMP;
  }
...
inline bool DominateLeft( const TUPLE &left, const TUPLE &right ) {
  ...
  for (; dim+8 <= NUM_DIMS; dim+=8) {
    ...
    __m256 comp_gt = _mm256_cmp_ps(left_ymm, right_ymm, 13);
    uint32_t gt_mask = _mm256_movemask_ps(comp_gt);
    if( gt_mask != 255 ) return false;
  }
  ...
  for (; dim+4 <= NUM_DIMS; dim+=4) {
    ...
    __m128 comp_gt = _mm_cmp_ps(left_xmm, right_xmm, 13);
    uint32_t gt_mask = _mm_movemask_ps(comp_gt);
    if( gt_mask != 15 ) return false;
  }
